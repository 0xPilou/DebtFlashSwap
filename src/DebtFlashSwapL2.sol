...
contract PairFlash is IUniswapV3FlashCallback, PeripheryImmutableState, PeripheryPayments {
    ...
    function swapFullDebt(address _debtToken, address _newDebtToken, uint8 _rateMode, uint24 _poolFee) external {
        _checkRateModeValidity(_rateMode);
        ...
        PoolAddress.PoolKey memory poolKey = PoolAddress.PoolKey({token0: _debtToken, token1: _newDebtToken, fee: _poolFee});
        IUniswapV3Pool pool = IUniswapV3Pool(PoolAddress.computeAddress(factory, poolKey));
        // recipient of borrowed amounts
        // amount of token0 requested to borrow
        // amount of token1 requested to borrow
    ...
    function _checkRateModeValidity(uint8 _rateMode) internal {
        if(_rateMode != 1 && _rateMode != 2) revert InvalidRateMode();
    }
